...
epi{1}(115:120,55:95)= c_eps_0*11.3;
epi{1}(125:130,55:95)= c_eps_0*11.3;
epi{1}(135:140,55:95)= c_eps_0*11.3;
epi{1}(145:150,55:95)= c_eps_0*11.3;

sigma{1} = zeros(nx{1},ny{1});
sigmaH{1} = zeros(nx{1},ny{1});

%Setting up the number of steps, etc.
dx = xMax{1}/nx{1};
dt = 0.25*dx/c_c;
...
%Setup boundary conditions and soft sources
bc{1}.NumS = 2;
...
%Setup parameters for that boundary condition
